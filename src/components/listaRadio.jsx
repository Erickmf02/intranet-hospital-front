function ListaRadio({value, setValue, list}){
    ...
    return (
        <>
            {list.map(item=>(
                <div className="w-100 d-flex align-items-center" style={{gap: "2px"}} key={item.key}>
                    <div className="d-flex align-items-center" key={item.key} style={{gap: "12px"}} onClick={()=>{handleOnChange(item.key)}}>
                        <input className="form-check-input m-0 ho_check_box" type="radio" checked={item.key==value} onChange={()=>{handleOnChange(item.key)}}/>
                        <div className="ho_font_poppins ho_fs_16">{item.texto}</div>
                    </div>
                    {item.help&&
